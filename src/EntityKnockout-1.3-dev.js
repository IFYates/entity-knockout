...
function EntityRepository(modelName, options) {
	...
	window[modelName + 'Model'].prototype = {
		...
		forJSON: function () {
	        ...
	        if (ko.utils.unwrapObservable(self.IsNew))
				obj.IsNew = true;
	        else if (ko.utils.unwrapObservable(self.IsDirty))
				obj.IsModified = true;
			return obj;
		},
		toJSON: function () {
	...
	//     callback (function) - called with (response data, bool success, dict errors)
	//			errors only sent if status 418 (I'm a Teapot) returned by server
	//     post (bool), defaults false
	//     errorObj (Entity), an entity to apply errors to
    self.Call = function (action, options, errorObj) {
		options = options || {};
		if (!_enabled)
			return;
		// Try to get from server
		...
		var done = function (data, success) {
			...
			var errors = null;
			if (data != null && typeof data == 'object' && ('status' in data) && data.status == 418) {
				var errs = $.parseJSON(data.responseText);

				errors = new Array();
				if (errorObj)
					errorObj.clearErrors();
				for (key in errs) {
					if (errs.hasOwnProperty(key)) {
						errors.push(errs[key]);
						if (errorObj)
							errorObj.setError(key, errs[key]);
					}
				}
			}
			...
			if (typeof options.callback == 'function')
				options.callback(data, success == 'success', errors);
		};
		self.IsBusy(true);
		...
		$.ajax({
			...
			success: done,
			error: done,
			type: (options.post == true ? 'POST' : 'GET'),
			dataType: (options.dataType != null ? options.dataType : null)
	...
	//    async (bool), defaults true - execute asynchronously
	//    params (dictionary) - key-value pairs to send as GET parameters
	//    callback (func(saved)) defaults undefined - called after all other callbacks
    ...
    var _explicitlyRemoveSubscribers = function (object) {
        for (var prop in object) {
            if (object[prop]) {
                if (typeof object[prop].valueWillMutate == "function")
                    _explicitlyRemoveSubscribers(object[prop])

                for (var s in object[prop]._subscriptions) {
                    for (var t in object[prop]._subscriptions[s]) {
                        if (typeof object[prop]._subscriptions[s][t].dispose == "function") {
                            object[prop]._subscriptions[s][t].dispose()
                        }
                    }
                }
            }
	...
	var _decorateModel = function (repo, self, data) {
		...
		self.setError = function (field, error) {
			if (ko.isObservable(self[field])) {
				if ('rules' in self[field]) {
    ...
    var _setObject = function (model, field, obj) {
        if (typeof obj == 'object' && obj != null && $.isFunction(model[field + '_NewItem'])) {
            model[field + '_NewItem'](obj);
        } else if ($.isFunction(model[field])) {
			model[field](obj);
		} else {
			model[field] = obj;
...
// TODO
EntityRepository.DefaultOptions = {
	...
	includeMissingModels: true, // TODO: If not model does not exist, try to include the model file
...
// TODO
// Return a repository for a model as defined by a server response.
// The server must return an array or a single POJO in JSON format.
// All fields in the @keys array will be defined non-observable, all
//  others will by observable (or observableArray, if necessary).
// @url - The URL to request
// @name - The model name to instanciate
// @keys - Array of field names that should be used as the unique key
// @options:
//     bool  async (false) - Be aware that the model type will not exist until the server has responded
//     array params - Sent as part of the request
//     function(repo) callback - Invoked with the repository instance at completion
EntityRepository.BuildFromServer = function (url, name, keys, options) {
};
